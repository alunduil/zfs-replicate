...
`--doctest-modules` (configured in [`pyproject.toml`](../../pyproject.toml));
`testpaths` is `zfs_test`. Coverage is off by default. Run
`pytest --cov --cov-report=term-missing` to measure it locally.
